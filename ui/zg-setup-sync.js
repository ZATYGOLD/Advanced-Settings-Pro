// Keeps the Natural Wonders count setting and the per-wonder Selection
// toggles consistent with each other in the game setup screen:
...
const COUNT_PARAM_ID = "ZG_NaturalWondersCount";
...
function syncNaturalWonderSetup() {
	const countParam = GameSetup.findGameParameter(COUNT_PARAM_ID);
	if (!countParam) {
		return;
	}
	...
	try {
		...
		if (lastTier != null && tier != lastTier) {
			...
			} else if (lastTier == TIER_DISABLED) {
		...
		if (enabled == 0 && tier != TIER_DISABLED) {
			setParamByName(COUNT_PARAM_ID, TIER_DISABLED);
			lastTier = TIER_DISABLED;
			return;
		}
...
setInterval(() => {
	...
	try {
		...
		console.warn(`ZG-ASP setup sync error: ${e}`);
	}
}, POLL_MS);
